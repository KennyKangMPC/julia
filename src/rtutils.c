                      ...
                      jl_bt_element_t *bt_data, size_t bt_size)
{
    ...
    jl_excstack_t *s = *stack;
    memcpy(jl_excstack_raw(s) + s->top, bt_data, sizeof(jl_bt_element_t)*bt_size);
    s->top += bt_size + 2;
    jl_excstack_raw(s)[s->top-2].uintval = bt_size;
    jl_excstack_raw(s)[s->top-1].jlvalue = exception;
}
