...
void *mach_profile_listener(void *arg)
{
    ...
    while (1) {
        ...
        for (i = jl_n_threads; i-- > 0; ) {
            ...
            if (forceDwarf == 0) {
                // Save the backtrace
                bt_size_cur += rec_backtrace_ctx((jl_bt_element_t*)bt_data_prof + bt_size_cur, bt_size_max - bt_size_cur - 1, uc);
            }
            else if (forceDwarf == 1) {
                bt_size_cur += rec_backtrace_ctx_dwarf((jl_bt_element_t*)bt_data_prof + bt_size_cur, bt_size_max - bt_size_cur - 1, uc);
            }
            else if (forceDwarf == -1) {
            ...
            bt_data_prof[bt_size_cur++].uintval = 0;
